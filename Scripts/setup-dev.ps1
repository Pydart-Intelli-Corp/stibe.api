...
if (Test-Path $secretsFile) {
    ...
    try {
        ...
        if (-not $missingSection) {
            Write-Host "`n🎉 All required sections are present!" -ForegroundColor Green
        } else {
            Write-Host "`n⚠️  Some sections are missing. Please check your secrets file." -ForegroundColor Yellow
        }
...
if (Test-Path "obj") {
    ...
    Write-Host "⚠️  NuGet packages may need to be restored" -ForegroundColor Yellow
    Write-Host "Run: dotnet restore" -ForegroundColor Yellow
}

Write-Host "`n🚀 To start the API in development mode:" -ForegroundColor Cyan
