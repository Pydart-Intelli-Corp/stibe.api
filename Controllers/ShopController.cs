    ...
    public class ShopController : ControllerBase
    {
        ...
        private void DeleteOldShopImage(string imageUrl)
        {
            try
            {
                ...
                {
                    // For relative URLs like "/uploads/shop-images/filename.jpg"
                    var pathSegments = imageUrl.Split('/');
                    fileName = pathSegments.Last();
                }

                var oldFilePath = Path.Combine(_environment.WebRootPath, "uploads", "shop-images", fileName);
                
                if (System.IO.File.Exists(oldFilePath))
                {
                    System.IO.File.Delete(oldFilePath);
                    _logger.LogInformation("Old shop image deleted successfully: {FilePath}", oldFilePath);
                }
                else
                {
                    _logger.LogWarning("Old shop image file not found: {FilePath}", oldFilePath);
                }
            }
            catch (Exception ex)
            {
