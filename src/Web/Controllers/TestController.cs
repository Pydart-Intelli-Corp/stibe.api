...
namespace stibe.api.Controllers
{
    ...
    public class TestController : ControllerBase
    {
        ...
        public TestController(ApplicationDbContext context, IFileService fileService)
        {
            _context = context;
        ...
        public async Task<ActionResult<ApiResponse>> FileSystemCheck([FromServices] IWebHostEnvironment environment)
        {
            try
            {
                var info = new
                {
                    ...
                    FileServiceType = _fileService?.GetType().Name ?? "Unknown",
                    AzureContainers = new[] 
                    { 
                ...
                // Test file creation (local temp for basic functionality)
                // Note: Primary file storage is now Azure Blob Storage
                try
                {
                    var tempDir = Path.GetTempPath();
                    var testFilePath = Path.Combine(tempDir, $"stibe_test_{Guid.NewGuid()}.txt");
                    await System.IO.File.WriteAllTextAsync(testFilePath, "Test file content for health check");
