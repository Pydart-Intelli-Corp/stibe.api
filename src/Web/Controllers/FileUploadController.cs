    ...
    public class FileUploadController : ControllerBase
    {
        ...
        public IActionResult ListFiles(string containerName)
        {
            try
            {
                ...
                // Azure Blob Storage - file listing is not directly available
                // This would require implementing Azure Blob listing functionality
                // For now, return a message indicating Azure storage is being used
                return Ok(new { 
                    success = true, 
                    message = "File listing not available for Azure Blob Storage containers",
                    note = "Files are stored in Azure Blob Storage. Use Azure Storage Explorer to browse files.",
        ...
        public IActionResult TestUploadConfiguration()
        {
            try
            {
                var result = new
                {
                    ...
                    Containers = new[]
                    {
                        "profile-images", "service-images", "shop-images", 
                        "product-images", "receipts", "apk-files"
                    },
        ...
        public async Task<IActionResult> DeleteFile(string containerName, string fileName)
        {
            try
            {
                ...
                var fileUrl = $"https://stibestorage.blob.core.windows.net/stibe-datas/{containerName}/{fileName}";
